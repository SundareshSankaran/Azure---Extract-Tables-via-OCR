...
input_type = str('url')                        # type of input: file, url 
input_mode = str('single')                       # single or batch
file_path = str('data/table-test-document.pdf') # path to a (single) file
file_url = str('https://raw.githubusercontent.com/Azure/azure-sdk-for-python/main/sdk/documentintelligence/azure-ai-documentintelligence/samples/sample_forms/receipt/contoso-receipt.png')
...
query_fields = str('City, First_name, CPU')      # string containing comma separated keys to extract
query_exclude_metadata = bool(False)             # if excluded, the resulting table will contain a column per query field (doesn't support ocr metadata like bounding boxes)
...
def query_to_list(query_list: str):
...
print(f'number of files: {file_list.shape[0]}')
