...
azure_key = ''
azure_endpoint = ''
local_ocr = False                     									# whether to use a locally deployed document intelligence container, default = False
local_ocr_endpoint = '' 				   								# endpoint of the locally deployed document intelligence container
...
# general
ocr_type = 'text'                          								# type of OCR: text, form, query, table
input_type = 'file'                     								# type of input: file, url 
input_mode = 'batch'                       								# single or batch
file_path = '' 															# path to a single file
file_url = '' 															# url to a single file
input_table_name = ''                  									# name of table containing the file paths
path_column = ''                            							# column that contains the file path
output_status_table = False     										# whether to output the status table

# advanced
locale = ''                               								# optional, language of the document. Support-list: https://learn.microsoft.com/en-us/azure/ai-services/document-intelligence/language-support-ocr?view=doc-intel-4.0.0&tabs=read-hand%2Clayout-print%2Cgeneral
n_threads = 16                            								# number of threads to use for parallel processing
n_con_retry = 3                   										# number of retries if connection fails
retry_delay = 5                           								# delay between retries
timeout_seconds = 30                    								# seconds waiting for ocr processing to finish before timing out
save_json = False                         								# whether to save the json output
json_output_folder = ''              									# folder to save the json output
...
query_fields = ''          			                              		# string containing comma separated keys to extract
query_exclude_metadata = True 											# if excluded, the resulting table will contain a column per query field (doesn't support ocr metadata like bounding boxes)

# for table extraction
table_output_format = 'map'            									# how the tables should be returned: map, reference*, table** *reference requires a cas
table_output_library = 'work'         									# caslib to store the table (only relevant if table_output_format = 'reference')
select_table = False                    								# whether to select a specific table or all tables (only relevant if table_output_format = 'reference')
table_selection_method = 'index'      									# how to select the table: size, index (only relevant if table_output_format = 'reference' and selected_table = True)
table_selection_idx = 0            										# index of the table to extract (only relevant if table_output_format = 'table')
